...
class UnstructuredSparse(nn.Module):
    ...
    def __init__(
        ...
        non_compressed_sparse_values: bool = False,
        ...
        Args:
            n_out (int): the input size
            n_in (int): the output size
            frac_sparse (float): the fraction of the input to keep in a sparse mask
            device (torch.device): the device
            pattern (Optional[Tuple[int, int]], optional): Pattern for N:M. Defaults to None.
            sparse_group (Union[int, Literal[&quot;n_in&quot;]], optional): whether to use input grouping like Wanda. Defaults to -1.
            non_compressed_sparse_values (bool, optional): whether to use non-compressed sparse values. Used when doing Permutation Stuff Defaults to False.
        """
        ...
        self.register_buffer("sparse_mask", sparse_mask)
        if non_compressed_sparse_values:
            self.sparse_values = nn.Parameter(
                torch.zeros((n_out, n_in), device=device)
            )
        else:
    ...
    def sparse(
        ...
        if self.sparse_values.shape == (self.n_sparse,):
            # if the sparse values are compressed, we need to update them
            self.sparse_values.data = remaining_weight[self.sparse_mask].clone().detach()
        else:
            self.sparse_values.data = remaining_weight.clone().detach() 
        # self.sparse_values.data = remaining_weight[new_mask].clone().detach()

    def update_fixed_mask(self, remaining_weight: torch.FloatTensor):
        # print("remaining_error", remaining_error)
        raise NotImplementedError(
            "update_fixed_mask is not implemented for UnstructuredSparse. Use sparse() instead."
        )
    ...
    def reconstruct(self):
        if hasattr(self, "cached_reconstruction"):
            ...
            if self.sparse_values.shape == (self.n_sparse,):
                ...
                return self.sparse_values * self.sparse_mask
    ...
    def compress_sparse_values(self):
        assert self.sparse_values.shape == (self.n_out, self.n_in), \
            f"sparse_values shape {self.sparse_values.shape} does not match expected shape {(self.n_out, self.n_in)}"
        self.sparse_values = nn.Parameter(
            self.sparse_values[self.sparse_mask].detach().clone()
        )
        
    def uncompress_sparse_values(self):
        assert self.sparse_values.shape == (self.n_sparse,), \
            f"sparse_values shape {self.sparse_values.shape} does not match expected shape {(self.n_sparse,)}"
        with torch.no_grad():
            out = torch.zeros(
                (self.n_out, self.n_in),
                device=self.sparse_values.device,
            ...
            out[self.sparse_mask] = self.sparse_values
        self.sparse_values = nn.Parameter(out.detach().clone())