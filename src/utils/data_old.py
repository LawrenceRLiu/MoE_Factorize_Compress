...
def get_c4(nsamples, seed, seqlen, model, train_test: str = "train"):
    ...
    if train_test == "train":
        ...
        random.seed(seed)
        trainloader = []
        for _ in tqdm.tqdm(range(nsamples), desc="Loading C4 train samples"):
            while True:
        ...
        random.seed(0)
        valenc = []
        for _ in tqdm.tqdm(range(256), desc="Loading C4 validation samples"):
            while True:
                i = random.randint(0, len(data) - 1)
...
def get_loaders(
    ...
    if "c4" in name:
        ...
        raise NotImplementedError(
            f"Dataset {name} not recognized. Please implement a new dataset loader."
        )
