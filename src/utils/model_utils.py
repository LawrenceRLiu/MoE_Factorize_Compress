...
def get_compressed_model_class(base_model: str):
    """
    Get the appropriate compressed model class based on the base model name.
    
    Args:
        base_model (str): The base model name/path
        
    Returns:
        The appropriate compressed model class
    ...
    elif base_model.startswith("google/gemma-2"):
        from ..model.gemma2 import Gemma2ForCausalLM
        return Gemma2ForCausalLM
    else:
        raise ValueError(
            f"Unknown model type {base_model}, please implement the model loading logic. "
            f"Supported models: meta-llama/, Qwen/Qwen2-, Qwen/Qwen3-, google/gemma-2, DeepSeek-R1-Distill-Qwen"
        )


def create_compressed_model(base_model: str, cfg: DictConfig, orig_config: Any = None):
    """
    Create a compressed model with the appropriate configuration.
    
    Args:
        base_model (str): The base model name/path
        cfg (DictConfig): Configuration containing compression settings
        orig_config (Any, optional): Original model config. If None, will load from base_model.
        
    Returns:
        The compressed model instance
    """
    ...
    compression_config = OmegaConf.to_container(cfg.compress, resolve=True)
    compression_config["add_bias"] = cfg.add_bias
    orig_config.compress_config = compression_config
    
    # Get the appropriate model class and create the model
    compressed_model_class = get_compressed_model_class(base_model)
    compressed_model = compressed_model_class(orig_config)
...
def inference_layer(
    ...
    for j in tqdm.tqdm(
        ...
        if offload_activations:
            inp = inps[j : j + batch_size].to(dev)
            if attention_mask is not None:
                attention_mask = attention_mask[j : j + batch_size].to(dev)

        if attention_mask is not None:
            out = layer(
                inp, attention_mask=attention_mask, **layer_kwargs
            )[0]
        else:
            out = layer(
                inp, **layer_kwargs
        ...
        if inplace:
            inps[j : j + batch_size] = out
        else:
            outs[j : j + batch_size] = out
